...
## 7. 互动模块核心代码分析
互动模块用于记录用户对商品的真实行为，是平台的活跃度核心来源。

### 7.1 行为类型定义

`InteractionEnum` 定义了三种互动：

- `SAVE`：收藏
- `VIEW`：浏览
- `LOVE`：想要

### 7.2 “想要”操作 `likeProduct`
```java
...
这个功能不仅记录了用户兴趣，还触发了站内通知，形成了“互动 + 消息”的联动机制。

---

### 7.3 收藏/取消收藏 `saveOperation`
```java
...
### 7.7 清空浏览记录 `batchDeleteInteraction`
```java
...
- `update(Category category)`：更新分类信息；
- `batchDelete(List<Integer> ids)`：批量删除分类。

### 10.3 分类查询

`query(CategoryQueryDto)` 是典型分页查询方法，用于前台或后台列表展示。

### 10.4 管理员权限控制

分类删除接口被 `@Protector(role = "管理员")` 保护，说明分类维护属于管理员权限操作。

---
## 11. 仪表盘模块核心代码分析
仪表盘模块用于后台首页统计与图表展示。

### 11.1 基础统计 `staticCount`
```java
...
### 11.2 商品上架情况 `productShelvesInfo(day)`
```java
...
这类接口很适合后台可视化页面，用于显示“最近 7 天商品发布量”等趋势图。
