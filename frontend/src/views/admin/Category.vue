  ...
  <el-row style="background-color: #ffffff; padding: 5px 0; border-radius: 5px">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer" style="margin-top: 10px">
        <span class="channel-button" @click="cannel()"> 取消操作 </span>
...
export default {
  ...
  methods: {
    ...
    // 修改商品类别信息
    updateOperation() {
      //写请求定位到商品类别的修改接口，这是后端提供的
      // /category/update ---put请求
      this.$axios
    ...
    // 商品类别新增
    addOperation() {
      //写请求定位到商品类别的新增接口，这是后端提供的
      // /category/save ---post请求
      this.$axios
    ...
    fetchFreshData() {
      ...
      this.$axios
        .post("/category/query", this.categoryQueryDto)
        .then((res) => {
          const { data } = res; //解构
          if (data.code === 200) {
